...
-- 从备份恢复数据，将created_by从原id映射到原uuid，使用新的id
...
SELECT t.`uuid`, t.`title`, t.`description`, t.`form_id`, t.`schema_json`, t.`status`, t.`version`, t.`start_time`, t.`end_time`, t.`max_submissions`, t.`allow_edit`, t.`allow_anonymous`, u.`id`, u.`name`, t.`create_time`, t.`update_time`, t.`publish_time`, t.`deleted`
...
SELECT a.`uuid`, t.`uuid`, u.`id`, COALESCE(a.`submitter_name`, u.`name`), a.`answer_data`, a.`total_score`, a.`submit_ip`, a.`user_agent`, a.`duration_seconds`, a.`create_time`, a.`deleted`
...
SELECT f.`uuid`, f.`original_name`, f.`stored_name`, f.`file_path`, f.`file_url`, f.`file_size`, f.`file_type`, f.`file_ext`, f.`file_hash`, u.`id`, u.`name`, f.`related_type`, f.`related_id`, f.`status`, f.`create_time`
...
SELECT REPLACE(UUID(), '-', ''), `trace_id`, `title`, `business_type`, `method`, `request_method`,
       (SELECT `id` FROM `sys_user_backup` WHERE `id` = `oper_user_id`),
